    ...
    def __iter__(self):
        ...
        while cons_label_index < len(self.cons_labels):
            ...
            while len(indices) < self.batch_size and cons_label_index < len(self.cons_labels):
                ...
                # ePillID guarantees 2 ref + >=1 consumer; CURE may have 1 (or 0) ref.
                # skip a consumer label with no reference (cannot form a ref/cons pair)
                if len(target_ref_indices) == 0 or len(target_cons_indices) == 0:
                    continue
                ...
                if self.batch_size - len(indices) < 4:
                    ...
                    for ref_label in neighbor_ref_labels:
                        ...
                        # ePillID has exactly 2 ref (front+back); CURE may have 1 -> use whatever exists
            ...
            # CURE: some classes lack a reference -> the inner loop can produce an
            # empty/degenerate batch. Skip those (empty batch -> collate crash).
            if len(indices) >= 2:
                yield indices
