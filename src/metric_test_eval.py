...
class MetricEmbeddingEvaluator(ModelEvaluator):
    ...
    def eval_model(self,
        ...
        # ePillID gallery covers every class contiguously; CURE may miss a class's
        # reference. Metrics below use unq_ref_labels (the classes actually present),
        # so a partial gallery is fine — warn instead of assert.
        if label_num != self.multihead_model.get_original_n_classes():
            import warnings
            warnings.warn(f"gallery max-label+1={label_num} != n_classes="
                          f"{self.multihead_model.get_original_n_classes()} (partial gallery)")
