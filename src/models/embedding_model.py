...
class EmbeddingModel(nn.Module):
    def __init__(self, network='resnet18', pooling='CBP', dropout_p=0.5, cont_dims=2048, pretrained=True, middle=1000, skip_emb=False):
        ...
        if network.startswith('convnext'):
            # Modern-backbone branch (reviewer #4): ConvNeXt with global average
            # pooling instead of CBP; produces a cont_dims embedding so the rest of
            # the metric-learning pipeline is unchanged and the fusion is comparable.
            ctor = {'convnext_tiny': models.convnext_tiny,
                    'convnext_small': models.convnext_small}[network]
            weights = 'DEFAULT' if pretrained else None
            cnx = ctor(weights=weights)
            feat_dim = {'convnext_tiny': 768, 'convnext_small': 768}[network]
            self.base_model = nn.Sequential(
                cnx.features,
                nn.AdaptiveAvgPool2d(1),
                nn.Flatten(),
                nn.Linear(feat_dim, cont_dims),
            )
        else:
