...
class CBP(nn.Module):
     ...
     def forward(self, x):
         ...
         for img in range(batchSize // bsn):
             ...
             # Ported to the torch>=1.8 FFT API (original used the removed
             # torch.fft()/torch.ifft() real/imag-pair calls). Math is identical:
             # FFT each count-sketch over projDim, complex-multiply, inverse FFT,
             # take the real part (Compact Bilinear Pooling via Tensor Sketch).
             sketch1 = batch_x.mm(self.sparseM[0].to(x.device))
             sketch2 = batch_x.mm(self.sparseM[1].to(x.device))
             fft1 = torch.fft.fft(sketch1.float(), dim=1)
             fft2 = torch.fft.fft(sketch2.float(), dim=1)
             tmp_y = torch.fft.ifft(fft1 * fft2, dim=1).real
